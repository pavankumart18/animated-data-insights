// States dashboard powered by JSON files in output/.
// Expects: output/states_all.json and output/states_details/<STATE>.json
/* globals d3 */
...
(function(){
  ...
  const stateIndex = new Map(); // code -> summary row from states_all.json
  ...
  function computeLeaves(rootData) {
    ...
    return root.leaves().map(d => ({
      key: safeId(d.data.cat || d.data.name || d.data.id),
      id: safeId(d.data.cat || d.data.name || d.data.id),
      name: d.data.name,
      value: d.value,
  ...
  async function loadStateDetails(code) {
    const res = await fetch(`output/states_details/${code}.json`);
    if (!res.ok) throw new Error(`Failed to load details for ${code}`);
    return res.json();
  }

  function stateOptionLabel(row){ return `${row.state}`; }
  ...
  function toTree(jobGroups, code){
    const groups = Array.isArray(jobGroups) ? jobGroups : [];
    const children = groups.map((g) => {
      const nm = String(g.name||'Other');
      return { id:`${code}_${safeId(nm)}`, name:nm, value: Math.max(0, +g.share || 0), cat:nm };
    });
  ...
  async function updateDashboard(code){
    const summary = stateIndex.get(code);
    if (!summary) return;
    document.getElementById('country-title').textContent = summary.state;
    const details = await loadStateDetails(code);
  ...
  function pickInitial(codes){
    const scored = codes.map(c => ({ code:c, n:(stateIndex.get(c)?.total_observations)||0 }));
  ...
  async function init(){
    // Reset SVG layer and (re)create group for states mode
    svg.selectAll('*').remove();
    prevLayout = new Map();
    g = d3.select('#treemap-svg').append('g').attr('class','tiles tiles-states');

    const res = await fetch('output/states_all.json');
    const all = await res.json();
    all.forEach(row => { if (row && row.state_code) stateIndex.set(String(row.state_code), row); });
    const codes = Array.from(stateIndex.keys()).sort();
    ...
    codes.forEach(code => {
      ...
      opt.value = code;
      opt.textContent = stateOptionLabel(stateIndex.get(code));
      select.appendChild(opt);
    });

    // URL-selected state takes precedence
    let initial = (getParam('state') || getParam('code') || '').toUpperCase();
    if (!codes.includes(initial)) initial = pickInitial(codes);
    select.value = initial;
    setParam('state', initial);
    ...
    window.addEventListener('popstate', () => {
      const code = (getParam('state') || getParam('code') || '').toUpperCase();
  ...
  // Expose for SPA and init only if mode=states
  const mode = (getParam('mode') || 'states').toLowerCase();
  window.AEI = window.AEI || {};
  window.AEI.states = { init, updateDashboard };
  if (mode === 'states') {
    init().catch(err => console.error('Failed to initialize states dashboard', err));
  }
})();
